...
import { Plus, Video, BookOpen, Users, ChevronRight } from 'lucide-react'
...
export default function AsatidzDashboard() {
  ...
  const firstName = profile?.nama?.split(' ')[0] || 'Ustadz'

  return (
    <div className="space-y-8">
      {/* Header Banner */}
      <div className="bg-[#064E3B] rounded-[40px] p-8 text-white relative overflow-hidden shadow-2xl">
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="space-y-2 text-center md:text-left">
            <h2 className="text-3xl font-black">Ahlan wa Sahlan, {firstName}!</h2>
            <p className="text-emerald-100/60 font-medium">Siap membagikan ilmu dan keberkahan hari ini?</p>
          </div>
          <Link
            href="/dashboard/asatidz/keilmuan/upload"
            className="bg-white text-[#064E3B] px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest flex items-center gap-3 hover:bg-emerald-50 transition-all"
          >
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link href="/dashboard/asatidz/live" className="text-emerald-600 font-bold text-xs uppercase tracking-widest">
                Lihat Semua
              </Link>
            </div>
        ...
        <div className="bg-white rounded-[40px] p-8 border border-gray-100 shadow-sm flex flex-col">
          ...
          <div className="mt-8 pt-6 border-t border-gray-100">
            <input
              ...
              className="w-full bg-gray-50 border-none rounded-2xl p-4 text-sm font-bold placeholder:text-gray-300 focus:ring-2 focus:ring-emerald-500 outline-none"
            />
          </div>
        </div>
      </div>
...
function LiveItem({ title, date, time }: { title: string; date: string; time: string }) {
  return (
    <div className="flex items-center justify-between p-4 rounded-3xl hover:bg-gray-50 transition-all border border-transparent hover:border-gray-100">
      <div className="flex items-center gap-4">
        <div className="w-16 h-12 bg-emerald-100 rounded-xl flex items-center justify-center text-emerald-600">
          <Video size={20} />
        </div>
        <div>
...
function ChatItem({ name, msg, time }: { name: string; msg: string; time: string }) {
  return (

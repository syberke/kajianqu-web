...
export default async function UstadzDetailPage({ params }: PageProps) {
  ...
  const [reviews, savedItem, ownReview, currentProfile, privateClasses, liveEvents, achievements] = await Promise.all([
    ...
    db.privateClass.findMany({ where: { asatidzId: ustadz.id, status: { in: ['published', 'open'] } }, orderBy: { createdAt: 'desc' }, select: { id: true, title: true, description: true, coverPath: true }, take: 12 }),
    db.liveEvent.findMany({ where: { asatidzId: ustadz.id, visibility: 'public', status: { not: 'cancelled' } }, orderBy: { startsAt: 'desc' }, select: { id: true, title: true, description: true, status: true, startsAt: true, thumbnailUrl: true }, take: 12 }),
    db.userAchievement.findMany({ where: { userId: ustadz.id, achievement: { isActive: true } }, orderBy: { awardedAt: 'desc' }, select: { achievement: { select: { id: true, title: true, icon: true } } }, take: 12 }),
  ])
    ...
    <main className="min-h-screen bg-[#f6faf8] pt-[72px]">
      <section className="bg-[#145c42] px-4 py-12 text-white sm:py-16">
        <div className="mx-auto grid max-w-6xl gap-8 sm:grid-cols-[160px_1fr_auto] sm:items-center">
          ...
          <div><p className="flex items-center gap-2 text-sm font-bold text-emerald-200"><ShieldCheck size={18} />Ustadz terverifikasi</p><h1 className="mt-2 text-4xl font-black">{ustadz.nama}</h1><p className="mt-2 text-lg text-white/75">{ustadz.asatidzProfile?.bidang || 'Keilmuan Islam'}</p><p className="mt-4 max-w-2xl leading-7 text-white/70">{ustadz.asatidzProfile?.bio || 'Pengajar KajianQu yang siap mendampingi proses belajar Anda.'}</p>{achievements.length > 0 && <div className="mt-4 flex flex-wrap gap-2">{achievements.map(({ achievement }) => <span key={achievement.id} className="rounded-full bg-white/10 px-3 py-1.5 text-xs font-bold text-emerald-50">{achievement.icon || '🏆'} {achievement.title}</span>)}</div>}</div>
      ...
      <div className="mx-auto max-w-6xl space-y-12 px-4 py-10 sm:px-6">
        ...
        <ClassSection title="Kelas private" empty="Belum ada kelas private yang dibuka." items={privateClasses.map((item) => ({ id: item.id, title: item.title, description: item.description, image: item.coverPath, href: '/kelas#private', icon: BookOpen }))} />
        <ClassSection title="Jadwal live" empty="Belum ada jadwal live." items={liveEvents.map((item) => ({ id: item.id, title: item.title, description: item.startsAt ? new Date(item.startsAt).toLocaleString('id-ID') : item.description, image: item.thumbnailUrl, href: `/kelas/live/${item.id}`, icon: CalendarDays, badge: item.status }))} />

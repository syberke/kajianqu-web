...
export default function AuthenticatedDonationForm() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      if (!proof) throw new Error('Bukti pembayaran wajib diunggah.')
      const paymentProofUrl = await uploadDonationProof(proof, user.id)
      const idempotencyKey = `web-donation-${user.id}-${crypto.randomUUID()}`

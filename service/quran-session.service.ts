...
export async function saveSession(session: Omit<QuranSession, 'id' | 'createdAt'>): Promise<string | null> {
  ...
  if (session.mistakes.length > 0) {
    const mistakeRows = session.mistakes.map((m: SessionMistake) => ({
      session_id: data.id,
      user_id: user.id,
      surah_id: session.surahId,
...
export async function getMostMistakenWords(limit = 10) {
  ...
  const freq: Record<string, { word: string; surahId: number; ayahNumber: number; count: number }> = {}
  data.forEach(row => {
    const key = `${row.surah_id}-${row.ayah_number}-${row.word_arabic}`
    if (!freq[key]) {
      freq[key] = { word: row.word_arabic, surahId: row.surah_id, ayahNumber: row.ayah_number, count: 0 }
    }
    freq[key].count++
  })
